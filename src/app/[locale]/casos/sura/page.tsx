...
import { suraContent } from "@/content/casos/sura-content";

type Props = { params: Promise<{ locale: string }> };

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "sura" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title,
    description,
    alternates: localeAlternates(locale, "/casos/sura"),
    ...pageOpenGraph(locale, title, description),
  };
}

// Case study CRM SURA — Agencia de Seguros.
// Contenido bilingüe via src/content/casos/sura-content.ts.
// Ambas rutas (/es/casos/sura y /en/casos/sura) son SSG — compiladas en build.
export default async function SuraPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const c = suraContent[locale as "es" | "en"] ?? suraContent.es;
  const tools = ["SmartSuite", "Gmail", "Gemini (Google AI)"];
  ...
  return (
    <>
      ...
      <Section className="bg-[color:var(--bg-secondary)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-[color:var(--border)]">
          {tools.map((tool) => (
